...
export default function DomainTreePage({
  ...
  useEffect(() => {
    ...
    async function loadExistingResult(projectId: string) {
      ...
      try {
        const response = await fetch(new URL(`/api/domain-tree/${encodeURIComponent(projectId)}`, apiBaseUrl));
        const payload = await response.json().catch(() => ({}));
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch(new URL("/api/domain-tree/generate", apiBaseUrl), {
        method: "POST",
        headers: {
      ...
      <section className="domain-tree-panel">
        ...
        {isLoadingPapers || isLoadingExisting ? (
          ...
          <div className="domain-tree-empty">
            ...
            <span>选中一篇已解析论文后，点击“生成领域树”开始。</span>
          </div>
        ) : (
