...
FIELDS = (
    ...
    EnvFieldSpec("AUXILIARY_MODEL_PROVIDER", "辅助模型供应商", "integrations", "text", "auxiliary_model_provider", "路由、查询规划和批量翻译使用；留空时回退主模型。"),
    EnvFieldSpec("AUXILIARY_MODEL_PROTOCOL", "辅助模型协议", "integrations", "choice", "auxiliary_model_protocol", "辅助模型接口协议。", options=("openai_compatible", "anthropic", "gemini", "ollama")),
    EnvFieldSpec("AUXILIARY_MODEL_NAME", "辅助模型名称", "integrations", "text", "auxiliary_model_name", "留空时所有任务继续使用主模型。"),
    EnvFieldSpec("AUXILIARY_MODEL_BASE_URL", "辅助模型 Base URL", "integrations", "text", "auxiliary_model_base_url", "辅助模型服务地址。"),
    EnvFieldSpec("AUXILIARY_MODEL_API_KEY", "辅助模型 API Key", "integrations", "secret", "auxiliary_model_api_key", "仅保存于环境配置，不写入日志。"),
    ...
    EnvFieldSpec("AGENT_MODEL_MAX_CONCURRENCY", "模型全局并发上限", "research", "integer", "agent_model_max_concurrency", "单次研究任务同时执行的模型调用上限。", 1, 8),
    EnvFieldSpec("RESEARCH_SEMANTIC_MAX_CONTEXT_CHARS", "语义验证字符预算", "research", "integer", "research_semantic_max_context_chars", "语义证据验证使用的总证据字符预算；预算会在证据组间公平分配。", 4000, 100000),
    EnvFieldSpec("RESEARCH_SEMANTIC_MAX_OUTPUT_TOKENS", "语义验证输出上限", "research", "integer", "research_semantic_max_output_tokens", "语义覆盖分类任务允许生成的最大 Token 数。", 512, 8192),
